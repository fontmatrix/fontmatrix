...
// 		return 0;
	...
	// As, AFAICS, lot of fonts use Any as I-do-not-care, return a penalty
	// if one of the font has it set to Any.
...
// 	int total(0);
	...
	{
		dr = pm->diff ( dbresult[i].second ) ;
		if(dr < 0)
			continue;
// 		totalMin = qMin(totalMin, dr);
// 		totalMax = qMax(totalMax, dr);
// 		totalMean += dr;
// 		++total;
		if ( dr < treshold )
		{
// 			selectMin = qMin(selectMin, dr);
// 			selectMax = qMax(selectMax, dr);
// 			selectMean += dr;
			fil << dbresult[i].first;
		}
	}
// 	qDebug()<<"TOTAL min/max/mean"<<totalMin << totalMax<< totalMean / total ;
// 	qDebug()<<"SELECT min/max/mean"<<selectMin << selectMax<< selectMean / fil.count();
	delete  pm;
	return fil;
}
